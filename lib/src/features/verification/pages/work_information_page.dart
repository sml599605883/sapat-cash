...
class WorkInformationPage extends StatefulWidget {
  const WorkInformationPage({super.key, required this.productId});

  static const routeName = RouteNames.workInformation;
...
class _WorkInformationPageState extends State<WorkInformationPage> {
  static const _retainPopupType = '3';
  ...
  final Map<int, String> _selectedChildFieldValues = <int, String>{};
  ...
  bool _loading = false;
  late int _scene6StartTimeSeconds;
  ...
  void initState() {
    super.initState();
    _scene6StartTimeSeconds = DateTime.now().millisecondsSinceEpoch ~/ 1000;
    _inactiveFocusNode = FocusNode(
      debugLabel: 'work_information_inactive_focus',
      skipTraversal: true,
    );
  ...
  Future<void> _loadData() async {
    ...
    EasyLoading.show();
    try {
      final response = await apiService.fetchWorkInfo(productId: productId);
  ...
  void _syncFieldControllers(List<PersonalInformationField> fields) {
    ...
    for (final id in staleIds) {
      ...
      _selectedChildFieldValues.remove(id);
  ...
  Future<void> _showSelectFieldSheet(PersonalInformationField field) async {
    ...
    _clearInputFocus();
    if (field.keyName == 'cookouts') {
      final selected = await showModalBottomSheet<_CascadedOptionSelection>(
        context: context,
        backgroundColor: Colors.transparent,
        isScrollControlled: true,
        builder: (sheetContext) {
          return _CascadedSelectFieldSheet(
            options: field.options,
            selectedParentValue: _selectedFieldValues[field.id] ?? field.value,
            selectedChildValue: _selectedChildFieldValues[field.id] ?? '',
          );
        },
      );
      _clearInputFocus();
      ...
      setState(() {
        _selectedFieldValues[field.id] = selected.parentValue;
        _selectedChildFieldValues[field.id] = selected.childValue;
      });
      return;
    }
  ...
  Map<String, dynamic> _buildSubmitFields() {
    ...
    for (final field in _model?.fields ?? const <PersonalInformationField>[]) {
      ...
      if (field.isSelectField) {
        final value = field.keyName == 'cookouts'
            ? _selectedChildFieldValues[field.id]
            : _selectedFieldValues[field.id];
  ...
  String _displaySelectValue(PersonalInformationField field) {
    ...
    final childValue = _selectedChildFieldValues[field.id] ?? '';
    final matchedChild = _matchSelectedOption(
    ...
    return '${matchedOption.label}|${matchedChild.label}';
  }
  ...
  Future<void> _submitUserInfo() async {
    ...
    EasyLoading.show();
    try {
      await apiService.saveWorkInfo(fields: submitFields);
      if (!mounted) {
        return;
      }
      if (productId.isNotEmpty) {
        ReportManager.instance.reportRiskBehavior(
          ...
          startTimeSeconds: _scene6StartTimeSeconds,
        );
  ...
  String _displayValue(PersonalInformationField field) {
    ...
    if (field.isSelectField) {
      return _displaySelectValue(field);
    }
    if (field.isAddressField) {
  ...
  Widget build(BuildContext context) {
    ...
    final hintMessage = _model?.hint.isNotEmpty == true
        ? _model!.hint
        : ProductDetailCache.current?.temporalize.workHint.trim() ?? '';
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Image.asset(
                      'assets/image/line/work_info_progress_bar.png',
                      width: width,
                      height: height,
                      fit: BoxFit.contain,
                    ),
...
class _CascadedOptionSelection {
  const _CascadedOptionSelection({
...
class _CascadedSelectFieldSheet extends StatefulWidget {
  const _CascadedSelectFieldSheet({
    required this.options,
    required this.selectedParentValue,
    required this.selectedChildValue,
  });

  final List<PersonalInformationOption> options;
  final String selectedParentValue;
  final String selectedChildValue;

  @override
  State<_CascadedSelectFieldSheet> createState() =>
      _CascadedSelectFieldSheetState();
}

class _CascadedSelectFieldSheetState extends State<_CascadedSelectFieldSheet> {
  int _level = 0;
  PersonalInformationOption? _selectedParent;
  PersonalInformationOption? _selectedChild;

  @override
  void initState() {
    super.initState();
    for (final option in widget.options) {
      if (option.value != widget.selectedParentValue) {
        continue;
      }
      _selectedParent = option;
      for (final child in option.options) {
        if (child.value == widget.selectedChildValue) {
  ...
  List<PersonalInformationOption> get _currentOptions {
    if (_level == 0) {
      return widget.options;
    }
    return _selectedParent?.options ?? const <PersonalInformationOption>[];
  }
  ...
  String get _titleText {
    ...
    return _selectedParent?.label ?? 'Select';
  }

  String get _actionLabel => _level == 0 ? 'Next' : 'Done';
  ...
  void _handleContinue() {
    ...
    Navigator.of(context).pop(
      _CascadedOptionSelection(
        parentValue: _selectedParent!.value,
        childValue: _selectedChild!.value,
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    final screen = context.screen;
    final bottomInset = MediaQuery.viewInsetsOf(context).bottom;
    final visibleCount = math.min(_currentOptions.length, 5);
    final listHeight =
        visibleCount * screen.dp(58) + (visibleCount - 1) * screen.dp(26);
    final maxHeight = screen.height * 0.45;
    return SafeArea(
          ...
          child: Column(
            ...
            children: [
              Align(
                alignment: Alignment.centerLeft,
                child: GestureDetector(
                  onTap: _level == 0 ? null : _handleBack,
                  behavior: HitTestBehavior.opaque,
                  child: Text(
                    _titleText,
                    style: TextStyle(
              ...
              ConstrainedBox(
                ...
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final selected = _level == 0
                        ? identical(option, _selectedParent)
                        : identical(option, _selectedChild);
                    return _SelectFieldOptionTile(
                      label: option.label,
                      selected: selected,
                      onTap: () {
              ...
              SizedBox(height: screen.dp(10)),
              _PrimaryButton(label: _actionLabel, onTap: _handleContinue),
            ],
          ),
        ),
      ),
